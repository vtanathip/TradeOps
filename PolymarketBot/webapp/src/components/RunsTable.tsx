    ...
    <div className="space-y-4">
      ...
      <div className="surface p-3 flex flex-wrap gap-3 items-center">
        <div className="flex-1 min-w-[200px] relative">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-600 pointer-events-none"
        ...
        <select className="input w-36 text-xs" value={strategy} onChange={e => setStrategy(e.target.value)}>
          {strategies.map(s => <option key={s} value={s}>{s === 'all' ? 'All strategies' : s}</option>)}
        </select>

        <select className="input w-32 text-xs" value={action} onChange={e => setAction(e.target.value)}>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-zinc-800">
                <th className="px-4 py-3 text-left text-[10px] font-semibold text-zinc-600 uppercase tracking-widest w-28">Time</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold text-zinc-600 uppercase tracking-widest">Market</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold text-zinc-600 uppercase tracking-widest w-28">Strategy</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold text-zinc-600 uppercase tracking-widest w-24">Action</th>
                <th className="px-4 py-3 text-right text-[10px] font-semibold text-zinc-600 uppercase tracking-widest w-20">YES</th>
                <th className="px-4 py-3 text-right text-[10px] font-semibold text-zinc-600 uppercase tracking-widest w-20">Limit</th>
                <th className="px-4 py-3 text-right text-[10px] font-semibold text-zinc-600 uppercase tracking-widest w-20">Size</th>
                <th className="px-4 py-3 text-right text-[10px] font-semibold text-zinc-600 uppercase tracking-widest w-20">Edge</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={s.id} className="hover:bg-zinc-800/30 transition-colors group">
                    ...
                    <td className="px-4 py-3 max-w-0 w-full">
                      ...
                      <p className="mono text-[10px] text-zinc-700 truncate">{s.condition_id?.slice(0, 12)}…</p>
                    </td>
                    <td className="px-4 py-3 text-zinc-500">{s.strategy ?? '—'}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className={`status-dot ${ac.dot}`} />
